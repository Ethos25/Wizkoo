/* global React, FAMILY, SUBJECTS_R7, SubjectR7, ChildR7, ChildChipR7,
   PrintCard, PrintHeader, NoteCard, PRINT_DEMO, RECAP_DEMO,
   PALETTE_J, PALETTE_K, PALETTE_L */

// =============================================================
// ROUND SEVEN — Renderers. Same vocabulary, parameterised by palette.
// =============================================================

function PhoneBandR7() {
  return (
    <div style={{
...
function SepR7() { return <span style={{ width: 1, height: 9, background: "#DCD6C5" }}/>; }
function CaretR7({ open }) {
  return (
...
// 1 · TODAY VIEW (expanded cluster · the brief's required composition)
function TodayViewR7({ palette }) {
  return (
    <div style={{
      ...
      <div style={{ marginTop: 18, padding: "0 22px" }}>
        ...
        ].map((r, i) => <SiblingRowR7 key={i} row={r}/>)}
      </div>
      <ExpandedSplitR7 palette={palette}/>
...
function ExpandedSplitR7({ palette }) {
  return (
    <div style={{ padding: "20px 0 22px" }}>
      ...
      <GroupBlockR7 palette={palette} label="GROUP 1" kidIds={["leo","max"]} blocks={[
        { time: 20, name: "Bridge Load Testing Documentation Lab" },
        { time: 20, name: "Bridge Construction Supply Problems" },
      ]}/>
      <GroupBlockR7 palette={palette} label="GROUP 2" kidIds={["jack","jane"]} blocks={[
...
function GroupBlockR7({ palette, label, kidIds, blocks }) {
  return (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center", gap: 12, marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {kidIds.map(id => {
            ...
            return <ChildR7 key={id} kid={k} palette={palette}/>;
          })}
        </div>
      </div>
      {blocks.map((b, i) => (
        ...
        {step.perChild.map((p, i) => {
          ...
          return (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "92px 1fr", columnGap: 12, alignItems: "center" }}>
              <ChildR7 kid={k} palette={palette} size={17}/>
    ...
    <div style={{
        ...
        {RECAP_DEMO.map(s => {
          ...
          return (
            <div key={s.kid} style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 10, alignItems: "center", padding: "10px 0" }}>
              <ChildR7 kid={k} palette={palette} size={20} hideName/>
      ...
      <div style={{ marginTop: 14, padding: "12px 14px", background: "var(--chalk)" }}>
        ...
        <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "center" }}>
          {FAMILY.map(k => <ChildChipR7 key={k.id} kid={k} palette={palette} size={20}/>)}
          <span style={{ width: 1, height: 18, background: "#E0DED6", margin: "0 6px" }}/>
          {Object.keys(SUBJECTS_R7).map(s => <SubjectR7 key={s} subject={s} dense/>)}
        </div>
      </div>
    </div>
  );
...
// 4 · PALETTE CARD
function PaletteCardR7({ palette }) {
    ...
    <div style={{
      ...
      <div style={{ marginTop: 14, padding: "10px 14px", background: "var(--paper)", border: "1px solid #ECE7DA" }}>
        ...
        <div style={{ marginTop: 8, display: "flex", gap: 10, alignItems: "center" }}>
          {kids.map(({ id, kid }) => <ChildChipR7 key={id} kid={kid} palette={palette} size={20}/>)}
          <span style={{ flex: 1 }}/>
          <span className="mono" style={{ fontSize: 8.5, color: "#8C91A5", letterSpacing: "0.18em" }}>FOUR HOLD</span>
        </div>
      </div>
    </div>
  );
...
// 5 · CONSTRAINT CHECK
function ConstraintCheckR7({ palette }) {
  return (
    <div style={{
      width: 620, height: 560, background: "var(--chalk)",
      padding: "30px 36px", boxSizing: "border-box",
      ...
      <h2 style={{ margin: "8px 0 0", fontFamily: "Fraunces", fontStyle: "italic", fontWeight: 500, fontSize: 26, letterSpacing: "-0.018em" }}>
        Four constraints. Defended explicitly.
      </h2>
...
// 6 · SYSTEM AUDIT — explicit per-axis check
function SystemAuditR7({ palette }) {
  const kids = ["leo","max","jack","jane"].map(id => ({
    id, kid: FAMILY.find(f => f.id === id), token: palette.family[id],
  }));
  return (
    <div style={{
      ...
      <div className="mono" style={{ fontSize: 10, letterSpacing: "0.22em", color: "#8C91A5", textTransform: "uppercase" }}>
        {palette.kicker} · SYSTEM AUDIT
      </div>
      <h2 style={{ margin: "8px 0 0", fontFamily: "Fraunces", fontStyle: "italic", fontWeight: 500, fontSize: 26, letterSpacing: "-0.018em" }}>
        Optimized as one set.
      </h2>

      <div style={{ marginTop: 18, borderTop: "1px solid #E0DED6" }}>
        {[
          ["CHROMATIC INTENSITY", palette.audit.chroma],
          ["VALUE RANGE",         palette.audit.value],
          ["TEMPERATURE",         palette.audit.temperature],
          ["FOUR TERRITORIES",    palette.audit.territories],
        ].map(([k, v], i) => (
          <div key={i} style={{
            padding: "14px 0",
            borderBottom: i === 3 ? "none" : "1px solid #ECE7DA",
            display: "grid", gridTemplateColumns: "auto 175px 1fr", columnGap: 14, alignItems: "baseline",
          }}>
            <span className="mono" style={{ fontSize: 10, letterSpacing: "0.20em", color: "#3CA85A" }}>✓ PASS</span>
            <span className="mono" style={{ fontSize: 9.5, letterSpacing: "0.20em", color: "var(--ink)" }}>{k}</span>
...
// 7 · NINE-COLOR LANDSCAPE
function NineColorLandscapeR7({ palette }) {
  const subj = Object.entries(SUBJECTS_R7);
  const kids = ["leo","max","jack","jane"].map(id => ({
    id, kid: FAMILY.find(f => f.id === id), token: palette.family[id],
  }));
  return (
    <div style={{
      ...
      <div style={{ marginTop: 18 }}>
        <div className="mono" style={{ fontSize: 9, letterSpacing: "0.22em", color: "#8C91A5" }}>FOUR CHILDREN · FOUR TERRITORIES</div>
        <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          {kids.map(({ id, kid, token }) => (
            <div key={id} style={{ background: "#FAFAFA", padding: "12px 12px 14px", border: "1px solid #ECE7DA" }}>
              <div style={{ height: 38, display: "flex", alignItems: "center" }}>
                <ChildChipR7 kid={kid} palette={palette} size={28}/>
              </div>
      ...
      <div style={{ marginTop: 18, padding: "16px 18px", background: "#FAFAFA", border: "1px solid #ECE7DA" }}>
        ...
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          {subj.map(([k]) => <SubjectR7 key={k} subject={k} dense/>)}
          <span style={{ width: 1, height: 22, background: "#0C1020" }}/>
          {kids.map(({ id, kid }) => <ChildR7 key={id} kid={kid} palette={palette} size={20}/>)}
        </div>
      </div>
    </div>
  );
...
// 8 · RATIONALE
function RationaleNoteR7({ palette }) {
  return (
    <NoteCard
...
function FrameCardR7() {
  return (
    <div style={{
      ...
      <div className="mono" style={{ fontSize: 10, letterSpacing: "0.22em", color: "#8C91A5", textTransform: "uppercase" }}>
        ROUND SEVEN · FOUR SLOTS OPEN
      </div>
      <h2 style={{ margin: "10px 0 0", fontFamily: "Fraunces", fontWeight: 600, fontSize: 30, lineHeight: 1.05, letterSpacing: "-0.018em" }}>
        Optimize as one system.
      </h2>
      <p style={{ marginTop: 12, fontSize: 12.5, lineHeight: 1.55, color: "var(--mid)", letterSpacing: "-0.003em", textWrap: "pretty" }}>
        Six rounds optimized subsets — three locked, one open, or four with anchors. Round seven opens all four. Each palette is selected as a SET, not assembled.
      </p>

      <div style={{ marginTop: 16, padding: "12px 14px", background: "#FAFAFA", border: "1px solid #ECE7DA" }}>
        <div className="mono" style={{ fontSize: 9, letterSpacing: "0.22em", color: "#8C91A5" }}>FOUR CONSTRAINTS</div>
        <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "auto 1fr", rowGap: 6, columnGap: 12, alignItems: "baseline" }}>
          {[
            ["01", "Hue territory — outside subject hue families at full chroma."],
            ["02", "Gender-neutral — no rose / pink / fuchsia; not all-military."],
            ["03", "No internal pairing — neither by hue nor by register."],
            ["04", "Visual equity at full chroma — every child carries colour."],
          ].map(([n, v]) => (
      ...
      <div style={{ marginTop: 14, padding: "12px 14px", background: "#FAFAFA", border: "1px solid #ECE7DA" }}>
        <div className="mono" style={{ fontSize: 9, letterSpacing: "0.22em", color: "#8C91A5" }}>SYSTEM AUDIT</div>
        <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "auto 1fr", rowGap: 6, columnGap: 12, alignItems: "baseline" }}>
          {[
            ["A", "Same chromatic intensity — matched saturation across all four."],
            ["B", "Distributed value range — light, mid, mid-dark, dark."],
            ["C", "Distributed temperature — warm / cool spread across slots."],
            ["D", "Four distinct hue territories — no adjacents."],
          ].map(([n, v]) => (
            <React.Fragment key={n}>
              <span className="mono" style={{ fontSize: 9, letterSpacing: "0.20em", color: "var(--ink)" }}>{n}</span>
              <span style={{ fontSize: 12, color: "var(--ink)", letterSpacing: "-0.003em", lineHeight: 1.45 }}>{v}</span>
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <div style={{ marginTop: 10, borderTop: "1px solid #E0DED6" }}>
          {[
            ["J · CERAMIC GLAZE",    "Wine / Clay / Slate / Oat — matched mid-chroma earthen pigments."],
            ["K · MINERAL CABINET",  "Madder / Ochre / Indigo / Porphyry — heritage natural pigments."],
            ["L · INKED PAPER",      "Bordeaux / Straw / Steelblue / Sage-Brown — matched low chroma quartet."],
          ].map(([k, v]) => (
            <div key={k} style={{
              padding: "10px 0", borderBottom: "1px solid #ECE7DA",
              display: "grid", gridTemplateColumns: "210px 1fr", gap: 14, alignItems: "baseline",
            }}>
...
function CompareStripR7() {
  return (
    <div style={{
      ...
      <h2 style={{ margin: "8px 0 0", fontFamily: "Fraunces", fontStyle: "italic", fontWeight: 500, fontSize: 26, letterSpacing: "-0.018em" }}>
        the screen the parent sees
      </h2>
      <p style={{ marginTop: 6, fontSize: 12, color: "var(--mid)", lineHeight: 1.5, letterSpacing: "-0.003em" }}>
        Expanded cluster, both groups. Each palette tested in the brief's required composition.
      </p>

      <div style={{ marginTop: 18, borderTop: "1px solid #E0DED6" }}>
        {[PALETTE_J, PALETTE_K, PALETTE_L].map((p, i) => (
          <div key={p.id} style={{ padding: "16px 0", borderBottom: i === 2 ? "none" : "1px solid #ECE7DA" }}>
            <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 18, alignItems: "center" }}>
              <div>
                ...
                <div className="mono" style={{ marginTop: 2, fontSize: 8.5, letterSpacing: "0.10em", color: "var(--faint)", textTransform: "none" }}>
                  {["leo","max","jack","jane"].map(id => p.family[id].fill).join(" · ")}
                </div>
              </div>
              <div style={{
                ...
                {["leo","max"].map(id => {
                  const k = FAMILY.find(f => f.id === id);
                  return <ChildR7 key={id} kid={k} palette={p}/>;
                })}
      ...
      <div style={{ marginTop: 12, padding: "12px 16px", background: "var(--chalk)", border: "1px solid #ECE7DA" }}>
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 24, alignItems: "center" }}>
          {[PALETTE_J, PALETTE_K, PALETTE_L].map(p => (
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {["leo","max","jack","jane"].map(id => {
                const k = FAMILY.find(f => f.id === id);
                return <ChildChipR7 key={id} kid={k} palette={p} size={20}/>;
              })}
            </div>
          ))}
        </div>
...
Object.assign(window, {
  TodayViewR7, BlockSheetR7, RecapBadgeR7, PaletteCardR7,
  ConstraintCheckR7, SystemAuditR7, NineColorLandscapeR7,
  RationaleNoteR7, FrameCardR7, CompareStripR7,
});
