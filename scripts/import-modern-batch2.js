...
async function enrichFromOL(title, author) {
  ...
  // Try 2: broader q= search if no result
...
async function enrich(title, author) {
  ...
  const isbn10 = ol ? ol.isbn10 : null;
  ...
  // 2. OL cover by ISBN
  if (isbn) {
    const isbnCover = await enrichFromOLbyISBN(isbn);
    if (isbnCover) {
      return {
        isbn,
        cover_image_url: isbnCover,
  ...
  // 3. No cover — return whatever we have
  return {
    isbn,
...
/* ── Main ───────────────────────────────────────────────────────────────────── */
async function main() {
  console.log('\n📚 Wizkoo Library — Modern Batch 2 Import\n');
  const pool = new Pool({ connectionString: DB_URL, ssl: { rejectUnauthorized: false }, max: 3 });
  await pool.query('SELECT 1');
  console.log('✓ Database connected\n');

  const raw = fs.readFileSync('C:/Users/amyog/Downloads/wizkoo_modern_batch2_50.csv', 'utf8');
  ...
  if (noCovers.length) {
    console.log('\n⚠ No cover after OL fallbacks (will patch via Amazon):');
