...
async function main() {
  ...
  const errors = [], noCovers = [], religionSkipped = [], verifySkipped = [];
  ...
  for (const cols of rows) {
    ...
    // Skip placeholder/unverified author rows
    const rawAuthor = get(COL.author);
    if (!rawAuthor || /VERIFY|TBD|PLACEHOLDER/i.test(rawAuthor)) {
      console.log('  [VERIFY SKIP] ' + title);
      verifySkipped.push(title);
      skipped++;
      continue;
    }
    ...
    // Religion check
    if (hasReligion(cols)) {
      console.log('  [RELIGION SKIP] ' + title);
      religionSkipped.push(title);
      skipped++;
      continue;
    }

    // Apply author override if needed
    const author = AUTHOR_OVERRIDES[titleKey] || rawAuthor;

    process.stdout.write('  ' + title.slice(0, 48).padEnd(50) + ' … ');

    const e = await enrich(title, author);
  ...
  const countRes = await pool.query("SELECT count(*) FROM library_books WHERE status='active'");
  await pool.end();
  console.log('\n' + '═'.repeat(60));
  console.log('✓ Imported:          ' + imported);
  console.log('⊘ Skipped:           ' + (skipped + dupes));
  console.log('📚 Total in library: ' + countRes.rows[0].count);
  console.log('═'.repeat(60));
  if (religionSkipped.length) { console.log('\nReligion skipped:'); religionSkipped.forEach(t => console.log('  • ' + t)); }
  if (verifySkipped.length)   { console.log('\nVerify/TBD skipped:'); verifySkipped.forEach(t => console.log('  • ' + t)); }
  if (noCovers.length) {
    console.log('\n⚠ No cover (patch via Amazon):');
