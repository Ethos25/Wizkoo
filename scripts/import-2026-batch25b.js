...
// ── Skips: unverified author/illustrator or explicit placeholder duplicates ──
const SKIP_TITLES = new Set([
  'how to have a thought: a walk with charles darwin',  // author/illus unverified
  'clothes to make you smile: patrick kelly designs his dreams', // author/illus unverified
  'black hair',                                          // duplicate of The Hair Book
]);

// ── Title corrections (placeholder row → real book data) ──
const TITLE_OVERRIDES = {
  'extreme birdwatching: after the rain - cora at the shore': {
    title:       "Cora's Beach Adventure",
...
/* ── Main ───────────────────────────────────────────────────────────────────── */
async function main() {
  console.log('\n📚 Wizkoo Library — 2026 Batch 25b Import\n');
  const pool = new Pool({ connectionString: DB_URL, ssl: { rejectUnauthorized: false }, max: 3 });
  await pool.query('SELECT 1');
  console.log('✓ Database connected\n');

  const raw = fs.readFileSync('C:/Users/amyog/Downloads/wizkoo_2026_batch_25b (1).csv', 'utf8');
  ...
  for (const cols of rows) {
    ...
    const rawTitle = get(COL.title);
    if (!rawTitle) continue;

    const titleKey = rawTitle.toLowerCase().trim();

    // Skip placeholder/unverified rows
    if (SKIP_TITLES.has(titleKey) || get(COL.author).includes('TITLE VERIFY') || get(COL.author).includes('PLACEHOLDER')) {
      console.log('  [SKIP] ' + rawTitle);
      skipped++;
      continue;
    }

    // Apply title/author corrections
    const override = TITLE_OVERRIDES[titleKey];
    const title       = override ? override.title       : rawTitle;
    const author      = override ? override.author      : get(COL.author);
    const illustrator = override ? override.illustrator : get(COL.illustrator);
    const publisher   = override ? override.publisher   : get(COL.publisher);
    ...
    try {
      ...
      `, [
        slug, title,
        author || null, illustrator || null,
        publisher || null, parseInt(get(COL.year_published), 10) || null,
